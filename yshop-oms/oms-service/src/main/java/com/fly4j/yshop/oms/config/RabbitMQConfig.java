...
public class RabbitMQConfig {
	...
	public Queue ttlQueue(){
		...
		arguments.put("x-message-ttl", 120000); // 单位:毫秒 1分钟仅仅用于测试，实际根据需求，通常30分钟或者15分钟
	...
	public Binding ttlBinding(){
		// 锁定库存后会发送一条routingKey=order.create的消息到ORDER-TTL-QUEUE,
		// 然后会被路由到延时队列ORDER-EXCHANGE,延时队列没有消费者,
		// 到期后会将消息转发
	...
	public Queue queue(){
		return new Queue("ORDER-CLOSE-QUEUE", true, false,
				false, null);
	}
