...
public class SmsSeckillSessionController extends BaseController {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "id", value = "秒杀活动时间段id", required = true, paramType = "path", dataType = "Integer"),
    })
    @GetMapping("/{id}")
    ...
    @PutMapping(value = "/{id}")
    public R update(@PathVariable("id") Integer id, @RequestBody SmsSeckillSession smsSeckillsession) {
    ...
    @ApiImplicitParam(name = "ids", value = "秒杀活动时间段id", required = true, paramType = "query", allowMultiple = true, dataType = "Integer")
    @DeleteMapping()
