...
@Service
@Slf4j
public class SkuSpecServiceImpl extends ServiceImpl<SkuSpecMapper, SkuSpecEntity> implements SkuSpecService {
    ...
    public void saveSkuSpecs(Long skuId, List<SpuForm.SpecValue> specList) {
        ...
        // 1. 直接删除该SKU的所有旧规格
        this.remove(new LambdaQueryWrapper<SkuSpecEntity>().eq(SkuSpecEntity::getSkuId, skuId));
        ...
        List<SkuSpecEntity> specsToSave = specList.stream()
                .map(spec -> {
                    SkuSpecEntity skuSpecEntity = new SkuSpecEntity();
