...
public class SpuServiceImpl extends ServiceImpl<SpuMapper, SpuEntity> implements SpuService {
  ...
  public SpuForm getSpuForm(Long spuId) {
    ...
    // 属性列表
    List<SpuForm.AttrValue> attrList = spuAttrService.listAttrsBySpuId(spuId)
      .stream()
      .map(item -> new SpuForm.AttrValue(item.getAttrName(), item.getAttrValue()))
      .toList();
    ...
    if (CollectionUtil.isNotEmpty(skuList)) {
      spuForm.setSkuList(skuList.stream().map(skuBO -> {
        ...
        // 规格列表
        List<SpuForm.SpecValue> specList = skuBO.getSpecList()
          .stream()
          .map(specBO -> new SpuForm.SpecValue(specBO.getSpecName(), specBO.getSpecName()))
          .toList();
